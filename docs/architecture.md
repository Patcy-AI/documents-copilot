...
flowchart LR
    ...
    claude[Anthropic Claude<br/>LLM generation]
    corpus[SEC filing corpus]
    ingestion[Ingestion pipeline<br/>download, parse, chunk,<br/>embed locally]
    ...
    backend -->|generate grounded answer| claude
    backend -->|stream answer + citations| browser

    corpus --> ingestion
    ingestion -->|store documents + chunks<br/>+ local embeddings| db
```
...
- Anthropic SDK (via PydanticAI) for generation
- Local sentence-transformers for embeddings (`BAAI/bge-small-en-v1.5`, 384-dim)
...
The frontend is responsible for user interaction, local UI state, and sending the authenticated user's request to the backend. It should never hold service-role credentials, run retrieval logic, call the LLM directly, or write privileged records to Supabase.
...
1. Embed the user's query with the configured local embedding model (`BAAI/bge-small-en-v1.5`, 384-dim).
...
- `vector(384)` embedding columns if the SQLAlchemy type renderer is not sufficient
...
- No direct LLM (Anthropic) calls from the browser.
